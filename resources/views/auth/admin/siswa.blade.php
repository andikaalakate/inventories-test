...
@section('body')
    ...
    <section class="p-6 my-4 mx-8 bg-[#2a2a2a] rounded-lg shadow-md">
            ...
            <table class="min-w-full bg-gray-300 h-full mb-4 border border-gray-300 shadow-sm rounded-lg overflow-hidden">
                <thead class="bg-[#1a1a1a]">
                    <tr class="border-b">
                        ...
                        <th class="py-2 px-4 text-left text-sm font-semibold text-white">NISN</th>
                        <th class="py-2 px-4 text-left text-sm font-semibold text-white">Nama</th>
                        <th class="py-2 px-4 text-left text-sm font-semibold text-white">Jenis Kelamin</th>
                        <th class="py-2 px-4 text-left text-sm font-semibold text-white">Kelas</th>
                        <th class="py-2 px-4 text-left text-sm font-semibold text-white">Jurusan</th>
                    ...
                    <x-splade-lazy>
                        ...
                        @forelse ($siswas as $index => $siswa)
                            <tr class="border-b hover:bg-[#3a3a3a]">
                                ...
                                <td class="py-2 px-4 text-sm text-white">{{ $siswa->nisn }}</td>
                                <td class="py-2 px-4 text-sm text-white">{{ $siswa->nama }}</td>
                                <td class="py-2 px-4 text-sm text-white">{{ $siswa->jenis_kelamin }}</td>
                                <td class="py-2 px-4 text-sm text-white">{{ $siswa->kelas }}</td>
                                <td class="py-2 px-4 text-sm text-white">{{ $siswa->jurusan }}</td>
                                <td class="py-2 px-4 text-sm text-white">
                                    <div class="justify-start items-center flex gap-4">
                                        <Link href="{{ route('admin.siswa.show', $siswa->nama) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-20 px-4 text-lg text-white text-center">Tidak ada Siswa</td>
                            </tr>
                        @endforelse
                    </x-splade-lazy>
                </tbody>
        ...
        <div class="flex justify-center w-full">
            @if ($siswas->hasPages())
                <x-pagination :paginator="$siswas" route="{{ route('admin.siswa.list') }}" />
            @endif
        </div>
    </section>
@endsection
